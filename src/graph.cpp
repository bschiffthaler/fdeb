...
void Graph::set_network_params(double edgeWeightThreshold_,
                               double edgePercentageThreshold_) {
...
void Graph::set_algorithm_params(double K_, int cycles_, int I0_,
                                 double compat_, double sigma_) {
  _K = K_;
  _cycles = cycles_;
  _I = I0_;
  _iter = _I;
...
void Graph::set_graphics_params(double alpha_) { _edgeOpacity = alpha_; }
...
void Graph::read(std::string nodesFile_, std::string edgesFile_) {
  ...
  for (int r = 1; r < rows; r++) {
    ...
    _nodes.insert(
        std::pair<std::string, Node>(std::string(labelCh), Node(x, y)));
  }
  ...
  for (int i = 0; i < edgesNum; i++) _edges[i]._width *= 1.0 / (wmax + 1.0);
...
void Graph::get_bounding_box(meerkat::mk_vector2 &bottomLeft_,
                             meerkat::mk_vector2 &topRight_, double frame_) {
  ...
  for (; it != _nodes.end(); it++) {
    x = it->second._pos.x();
    y = it->second._pos.y();
    if (x < bottomLeft_.x()) bottomLeft_.set_x(x);
    if (y < bottomLeft_.y()) bottomLeft_.set_y(y);
    if (x > topRight_.x()) topRight_.set_x(x);
    if (y > topRight_.y()) topRight_.set_y(y);
  }
...
#pragma omp critical
    {
      for (int ii = 0; ii < tmp.size(); ii += 2) {
        auto& xi = tmp[ii];
        auto& xj = tmp[ii + 1];
        compEdgePairs++;
        _edges[xi]._compatibleEdges.push_back(xj);
        _edges[xj]._compatibleEdges.push_back(xi);
      }
      if (edgesNum >= 100 && numProcessed % (edgesNum / 100) == 0) {
        _log.i("build_compatibility_lists", "%i%% done, compatible edges: %i",
               numProcessed / (edgesNum / 100), compEdgePairs);
      }
    }
  }
}
...
int Graph::iterate() {
  ...
  std::vector<std::vector<meerkat::mk_vector2> > forces(
      edgesNum,
      std::vector<meerkat::mk_vector2>((int)_edges[0]._subdivs.size(),
...
#pragma omp parallel for
  for (int i = 0; i < edgesNum; i++) {
    ...
    for (int j = 0; j < compatibleEdgesNum; j++)
      _edges[i].add_electrostatic_forces(
          forces[i], _edges[_edges[i]._compatibleEdges[j]], _edgeDistance);
  }
...
#pragma omp parallel for
    for (int i = 0; i < edgesNum; i++)
      _edges[i].add_gravitational_forces(forces[i], _gravitationCenter,
...
void Graph::add_subvisions() {
  ...
  int edgesNum = (int)_edges.size();
  for (int i = 0; i < edgesNum; i++) _edges[i].add_subdivisions();
}

void Graph::smooth() {
  _log.i("smooth", "applying Gaussian smoothing");
  int edgesNum = (int)_edges.size();
  for (int i = 0; i < edgesNum; i++) _edges[i].smooth(_smoothWidth);
  _log.i("smooth", "done");
}
...
void Graph::draw() {
  ...
  for (int i = 0; i < numEdges; i++) _edges[i].draw(_edgeOpacity);
